# 围观 计划 3 — poster POV 透镜 + X 皮肤 + 流式播放 实现计划

> **给 agentic 实现者：** 必须使用 `superpowers:subagent-driven-development`（推荐）或 `superpowers:executing-plans`，按任务逐项实现本计划。步骤使用复选框（`- [ ]`）语法跟踪。
> **审核锚点**：遵守 `2026-07-01-weiguan-conventions-and-contracts.md` §1。每个任务 = 锚点 `P3-T<n>`；实现打 `// review:P3-T<n>`、commit trailer `Review-Anchor: P3-T<n>`、验收测试打 `// review:P3-T<n>-AC<k>`。

**目标：** 把后端流来的增量 RunSnapshot，经"poster 视角纯函数"变成 ViewModel，再用**以假乱真的 X 皮肤**逐条流式播放（种子帖 + 评论刷出 + 计数），完成第一人称沉浸的进行时体验。

**架构：** 四段：TS 规范类型 + `applyDelta` 累加器（纯）→ `posterView` POV 透镜（纯）→ `useRunStream` 钩子（消费契约 §2.3 的 SSE，可注入 EventSource 供测试）→ X 皮肤组件 + `LiveScreen` 装配。皮肤**只消费 ViewModel**，绝不碰 RunSnapshot/后端（契约纪律）。

**技术栈：** React 18，TypeScript，Tailwind（复用 F0 tokens），Vitest + @testing-library/react（含 `renderHook`）。

## 全局约束
- 承接 计划 F0（壳/tokens/占位屏）与 计划 2 契约 §2.3（SSE 事件名与字段）、§2.6（POV→ViewModel）。
...
- 本计划**不做**：写内容/发起运行（计划 4）、点头像追问抽屉与复盘（计划 5）。头像 `onActorClick` 先留 prop，不接抽屉。
...
### 任务 1 (P3-T1): TS 规范类型 + applyDelta 累加器

**文件：** Create `frontend/src/model/canonical.ts`、`frontend/src/model/accumulate.ts`；Test `frontend/src/model/canonical.test.ts`.

**接口 — 产出：**
- TS 镜像后端规范模型：`Actor/Post/Reply/Reaction/Follow/Report/TraceEvent/RunSnapshot` 及字面量联合 `Platform/PostKind/ReactionKind/TargetType`（字段名与 计划 1 完全一致）。
...
- [ ] **步骤 1：写失败测试 `model/canonical.test.ts`**
```ts
...
- [ ] **步骤 2：运行确认失败** — `cd frontend && npm test -- canonical` → FAIL。
...
- [ ] **步骤 4：运行确认通过** — `npm test -- canonical` → PASS（2 passed）。
- [ ] **步骤 5：提交**
```bash
...
### 任务 2 (P3-T2): poster POV 透镜

**文件：** Create `frontend/src/pov/poster.ts`；Test `frontend/src/pov/poster.test.ts`.
...
- [ ] **步骤 2：运行确认失败** — `npm test -- poster` → FAIL。

- [ ] **步骤 3：写实现 `pov/poster.ts`**
```ts
...
- [ ] **步骤 4：运行确认通过** — `npm test -- poster` → PASS（4 passed）。
- [ ] **步骤 5：提交**
```bash
...
**文件：** Create `frontend/src/skins/x/{XAvatar,XActionBar,XPost,XReply,XFeed}.tsx`；Modify `frontend/src/index.css`（加 fadein）；Test `frontend/src/skins/x/xskin.test.tsx`.

**接口 — 产出：**（皮肤只吃 ViewModel）
...
- [ ] **步骤 1：追加 fadein 到 `frontend/src/index.css`**
```css
@keyframes fadein { from { opacity: 0; transform: translateY(4px); } to { opacity: 1; transform: none; } }
```

- [ ] **步骤 2：写失败测试 `skins/x/xskin.test.tsx`**
```tsx
...
- [ ] **步骤 3：运行确认失败** — `npm test -- xskin` → FAIL。
...
- [ ] **步骤 5：运行确认通过** — `npm test -- xskin` → PASS（4 passed）。
- [ ] **步骤 6：提交**
```bash
...
### 任务 4 (P3-T4): useRunStream 钩子（SSE，可注入）

**文件：** Create `frontend/src/api/runStream.ts`；Test `frontend/src/api/runStream.test.ts`.
...
- [ ] **步骤 1：写失败测试 `api/runStream.test.ts`**
```ts
...
- [ ] **步骤 2：运行确认失败** — `npm test -- runStream` → FAIL。

- [ ] **步骤 3：写实现 `api/runStream.ts`**
```ts
...
- [ ] **步骤 4：运行确认通过** — `npm test -- runStream` → PASS（2 passed）。
- [ ] **步骤 5：提交**
```bash
...
### 任务 5 (P3-T5): LiveScreen 装配 + 悬浮控制条

**文件：** Modify `frontend/src/screens/LiveScreen.tsx`（替换 F0 占位）；Test `frontend/src/screens/LiveScreen.test.tsx`.

**接口 — 产出：** `LiveScreen({ streamFactory? })`：读路由 `:id` → `useRunStream` → `posterView` → `<XFeed>`；悬浮控制条显示 `第 step/total 步` + 暂停/加速占位 + "看结果 →"（`status==="done"` 才可点，跳 `/run/:id/retro`）。

- [ ] **步骤 1：写失败测试 `screens/LiveScreen.test.tsx`**
```tsx
...
- [ ] **步骤 2：运行确认失败** — `npm test -- LiveScreen` → FAIL。

- [ ] **步骤 3：写实现 `screens/LiveScreen.tsx`**
```tsx
...
```
> 注：暂停/加速本计划仅占位（不改变流速）；真正的播放节流留到后续（不影响契约）。

- [ ] **步骤 4：运行确认通过** — `npm test -- LiveScreen` → PASS（2 passed）。
- [ ] **步骤 5：回归全部前端测试** — `cd frontend && npm test` → 全绿（F0 + P3 全部）。
- [ ] **步骤 6：提交**
```bash
...
## 自审
- **规格覆盖**：实现 设计 §2 第一人称 poster 视角、§3 核心循环第 4 步"进行时流式回放"、§6.1 X 皮肤（tabular 计数、评论 fadein 刷出、以假乱真无品牌）、§7.3 进行时线框（信息流 + 悬浮控制条）。契约 §2.3（SSE）与 §2.6（POV→ViewModel）落地并定稿。
- **占位符扫描**：无 TBD；暂停/加速为**有意占位**（已注明，不影响契约），其余均有完整代码与命令。
- **类型一致性**：TS `RunSnapshot` 字段与 计划 1 pydantic 完全同名；`PosterViewModel` 在 poster.ts 定义、被 XFeed/LiveScreen 消费；`EventSourceFactory` 在 runStream 定义、LiveScreen 透传；皮肤只依赖 ViewModel（不 import canonical 之外的后端概念）。
