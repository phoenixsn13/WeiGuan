# 围观 — 约定与接口契约（Conventions & Contracts）

> 日期：2026-07-01
> 作用：所有 Plan 的共同底座。①定义**审核锚点**机制（让设计者能按锚点审核 codex 的实现）；②**冻结**各 Plan 之间的接口契约（Ports & Adapters 的"缝"）。
> 纪律：任何一层只准依赖本文件冻结的契约，不得越过缝去依赖别层的内部实现。

---

## 1. 审核锚点机制（Review Anchors）
...
- 例：`P2-T3` = Plan 2 的 Task 3；`P2-T3-AC1` = 该任务第 1 条验收标准。
...
2. 对照该 Plan 末尾的 **审核索引表**，逐条确认：每个 `AC` 都有对应测试锚点、且断言与索引表描述一致。
3. 运行索引表里列的命令，核对预期输出。
4. 缺锚点 / 锚点与索引表不符 / 命令不过 → 记为审核未通过，退回。

### 1.4 每个 Plan 必带「审核索引表」
格式（放在每篇 Plan 末尾）：
...
> Plan 1 追溯适用本机制：其锚点为 `P1-T1`(规范模型) `P1-T2`(fixture) `P1-T3`(actors/posts/replies) `P1-T4`(reactions/follows/reports/traces)。交接时请 codex 按 §1.2 补打标记。

---

## 2. 冻结的接口契约（Frozen Contracts）

### 2.1 规范模型（已在 Plan 1 冻结）
`weiguan.canonical` 的 `RunSnapshot / Actor / Post / Reply / Reaction / Follow / Report / TraceEvent` 及枚举。**所有后续层的数据形状以此为唯一真源。** 字段见 Plan 1。

### 2.2 运行发起 REST（Plan 2 实现，Plan 4 消费）
```
POST /api/runs
Headers:
...
### 2.3 事件流 SSE（Plan 2 实现，Plan 3 消费）
```
...
### 2.4 追问 INTERVIEW REST（Plan 2 提供端点，Plan 5 消费）
```
...
### 2.5 快照 REST（Plan 2 实现，Plan 5/复盘消费）
```
...
### 2.6 POV → ViewModel 形状（Plan 3 冻结细节，此处先定骨架）
...
```
> 具体字段在 Plan 3 定稿；此处冻结的是"POV 是纯函数、皮肤只消费 ViewModel、皮肤不碰 RunSnapshot/引擎"这一约束。
...
1. 本文件（约定+契约）✅
2. Plan 2：后端引擎封装 + 逐步运行 + SSE + 运行/追问/快照 API（实现 §2.2–2.5）
3. Plan F0：前端骨架 + 产品壳 + 设计 tokens（§6 视觉语言落成 Tailwind theme）
4. Plan 3：poster POV 透镜 + X 皮肤 + SSE 流式播放（消费 §2.3、§2.6）
5. Plan 4：圈子卡牌画廊 + 自定义受众 + BYOK 设置（消费 §2.2）
6. Plan 5：INTERVIEW 抽屉 + 复盘上帝视角（消费 §2.4、§2.5）
```
```
