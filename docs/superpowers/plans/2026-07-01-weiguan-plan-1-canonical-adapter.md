# 围观 Plan 1 — 规范模型 + OASIS Adapter Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 把 OASIS 跑出来的 SQLite 事件日志，稳定地归一化成一个平台中立的「规范模型」（RunSnapshot），作为整个围观应用的稳定内核。

**Architecture:** Ports & Adapters。本 Plan 只做内核最里层：`canonical`（pydantic 规范模型）+ `adapter`（OASIS SQLite → RunSnapshot）。不碰引擎运行、不碰 LLM、不碰前端。用一个"按 OASIS 真实 schema 手工插入已知数据"的 golden fixture DB 做确定性测试。

**Tech Stack:** Python 3.10+，pydantic v2，pytest。（后端整体是 Python + FastAPI + SSE，但本 Plan 不引入 FastAPI。）

## Global Constraints

- Python ≥ 3.10（用 `list[X]` / `X | None` 语法）。
- pydantic v2（`model_dump()` / `Field(default_factory=...)`）。
- 本 Plan **不 import `oasis` 包**：fixture 用内联 SQL 自建，保证测试无需装 OASIS、无需 LLM、无需网络。
- 所有 `created_at` 一律以**原样字符串**保存，不做时区/解析转换（确定性优先）。
- 规范模型字段命名一旦定义不可改：后续 Plan（POV/皮肤/复盘）依赖这些精确名字。
- 代码放在仓库 `backend/` 下；测试放 `backend/tests/`；`pytest` 在 `backend/` 目录运行。
- OASIS 真实表结构（本 Plan 依据，逐字）：
...
backend/
  ...
  weiguan/
    __init__.py                          # Task 1
    canonical/
      __init__.py                        # Task 1 (re-export 模型)
      models.py                          # Task 1 —— 所有规范模型
    adapter/
      __init__.py                        # Task 3
      oasis_adapter.py                   # Task 3+4 —— SQLite → RunSnapshot
  tests/
    conftest.py                          # Task 2 —— fixture builder
    canonical/test_models.py             # Task 1
    adapter/test_oasis_adapter.py        # Task 3+4
```
...
- Create: `backend/weiguan/__init__.py`（空文件）
- Create: `backend/weiguan/canonical/__init__.py`
- Create: `backend/weiguan/canonical/models.py`
- Test: `backend/tests/canonical/test_models.py`

**Interfaces:**
- Produces（后续所有 Plan 依赖这些精确名字与类型）：
...
`backend/weiguan/__init__.py` 与 `backend/weiguan/canonical/__init__.py` 先建为空文件（Step 5 再回填 canonical 的 re-export）。

- [ ] **Step 3: 写失败测试 `backend/tests/canonical/test_models.py`**
...
Run: `cd backend && python -m pytest tests/canonical/test_models.py -v`
Expected: FAIL —— `ImportError: cannot import name 'Actor' from 'weiguan.canonical'`

- [ ] **Step 5: 写实现 `backend/weiguan/canonical/models.py`**
...
### Task 2: OASIS-schema golden fixture 构造器

**Files:**
- Create: `backend/tests/__init__.py`（空，便于包导入）
- Create: `backend/tests/conftest.py`
- Test: 同文件内自测（见 Step 3）
...
- [ ] **Step 1: 写 `backend/tests/__init__.py`**

空文件。

- [ ] **Step 2: 写 `backend/tests/conftest.py`（含建库 SQL 与已知数据）**
...
- [ ] **Step 3: 写并运行 fixture 自测（确认建库成功）**

在 `backend/tests/adapter/test_oasis_adapter.py` 先放一个占位自测（Task 3 会扩充）：
...
Run: `cd backend && python -m pytest tests/adapter/test_oasis_adapter.py -v`
...
### Task 3: Adapter 核心（actors / posts / replies）

**Files:**
- Create: `backend/weiguan/adapter/__init__.py`
- Create: `backend/weiguan/adapter/oasis_adapter.py`
- Test: `backend/tests/adapter/test_oasis_adapter.py`（追加）

**Interfaces:**
- Consumes：`weiguan.canonical` 全部模型；`oasis_db_path` fixture。
- Produces：
  - `load_run_snapshot(db_path: str, platform: Platform = Platform.TWITTER, seed_post_id: int | None = None) -> RunSnapshot`
  - Task 3 只填 `actors / posts / replies`；`reactions/follows/reports/traces` 在 Task 4 填。

- [ ] **Step 1: 写失败测试（追加到 `test_oasis_adapter.py`）**
...
Run: `cd backend && python -m pytest tests/adapter/test_oasis_adapter.py -v`
Expected: FAIL —— `ModuleNotFoundError: No module named 'weiguan.adapter'`

- [ ] **Step 3: 写实现 `backend/weiguan/adapter/oasis_adapter.py`（含空 `__init__.py`）**
...
Run: `cd backend && python -m pytest tests/adapter/test_oasis_adapter.py -v`
Expected: PASS（4 passed，含 Task 2 的 fixture 自测）
...
### Task 4: Adapter 关系（reactions / follows / reports / traces）

**Files:**
- Modify: `backend/weiguan/adapter/oasis_adapter.py`
- Test: `backend/tests/adapter/test_oasis_adapter.py`（追加）

**Interfaces:**
- Consumes：Task 3 的 `load_run_snapshot` 与全部规范模型。
- Produces：`load_run_snapshot` 填齐 `reactions`（来自 like/dislike/comment_like/comment_dislike 四表）、`follows`、`reports`、`traces`。字段语义：
...
Run: `cd backend && python -m pytest tests/adapter/test_oasis_adapter.py -k "reactions or follows or traces" -v`
Expected: FAIL —— `AssertionError`（`snap.reactions` 为空 `[]`）
...
- [ ] **Step 4: 运行全部 adapter 测试，确认通过**

Run: `cd backend && python -m pytest tests/ -v`
Expected: PASS（全部通过：models 3 + fixture 1 + adapter 3 + relations 3）
...
- `RunSnapshot.model_dump()` 可序列化为 JSON（供后续 Plan 存快照、流式、复盘复用）。

## 交接给下一个 Plan 的接口

后续 Plan（引擎运行+流式、POV 透镜、皮肤、复盘）一律**只依赖 `weiguan.canonical` 与 `load_run_snapshot`**，不得直接读 OASIS 表——这是 Ports & Adapters 的核心纪律。

---

## Self-Review

- **Spec 覆盖**：本 Plan 只对应 spec §1 的「Engine 封装 → Adapter → 规范模型」中的 Adapter+规范模型两层；Engine 实际运行（`oasis.make`/`env.step` 流式）刻意留到 Plan 2（需要 LLM/真跑，测试策略不同）。规范模型字段覆盖 spec §1 canonical 列出的 Actor/Post/Reply/Reaction/Repost(=Post.kind)/Follow/Report/Step(=TraceEvent+created_at)。
- **占位符扫描**：无 TBD/TODO；每个代码步给了完整代码与可运行命令。
- **类型一致性**：`load_run_snapshot` 签名、各模型字段名（如 `author_id`、`num_followers`、`kind`）在 Task 1/3/4 中一致；测试断言与模型字段逐一对应。
