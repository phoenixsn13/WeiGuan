...
>
> **锚点**：新计划码 `PA`。每个任务打 `# review:PA-T<n>`、commit trailer `Review-Anchor: PA-T<n>`、可测的验收打 `# review:PA-T<n>-AC<k>`。规则同前：TDD、不改设计、不改弱断言绕过、频繁提交。
...
### 产出：缺陷记录表（PA-T4 交付物）
在本文件末尾"手测记录"区，用表格记录每个问题：`屏/步骤 | 现象 | 期望 | 严重度(阻断/一般/观感) | 截图 | 建议归属(前端/后端/契约/引擎)`。**不在本轮就地大改**——手测只负责发现与归类；修复按严重度另开任务（阻断项可当场小修并注明）。
