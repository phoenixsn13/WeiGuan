...
版本日期：2026-07-06  
适用范围：P14「信息架构与产品接线贯通」之后的当前产品形态。覆盖发起、世界、历史、单平台评论区、多平台现场、身份页、单 run 复盘、launch 复盘、入口接线审计和自动化验收。  
推荐数据目录：`WEIGUAN_WORKDIR=/tmp/weiguan-e2e`。  
LLM 注意：本文以只读验收和界面接线为主；点击“开始围观”或“重新生成建议”会调用 LLM 或本地 OpenAI compatible 服务。

## 1. 本版重点

P14 在 P13 的页面产品化基础上，补齐“世界”和“身份”的产品接线：

- 世界是一级入口：`/worlds` 直接读取持久化世界，不再只从历史 launch 侧推。
- 发起会留在世界里：单平台和多平台发起都应能在世界/历史中继续到达。
- 可见世界名来自后端 `resolve_world_name`，用户可见区不展示裸长 ID。
- 发起页支持新建世界和继续世界，继续列表可搜索、窗口化，不无限铺开。
- 世界总览、历史记录、评论区、多平台现场中的人物名，在有明确归属时可进入身份页。
- 无人物归属时不强造身份入口，避免把内部编号暴露给用户。
- 入口审计制度化：见 [P14 入口接线审计](2026-07-06-weiguan-P14-wiring-audit.md)。

P14 的设计审核重点：

- 页面心智仍是社交媒体产品，不出现技术工作台感。
- 一级导航保持 `发起 / 世界 / 历史`。
- 所有用户入口都能继续前进，不出现看得到但点不通的关键内容。
- 可见按钮、标题、卡片、通知、热榜、身份名不展示裸 `world_id/person_id/account_id/run_id`。
- P14 新入口与 P13 原型调性一致，不另起视觉语言。
...
| 发起 | `/compose` | `/api/worlds`, `/api/runs/preview-cost`, `/api/identities`, `/api/persons`, `/api/runs`, `/api/multi-runs` | 可新建/继续世界；继续列表可搜索窗口化；成功后跳现场 |
...
参考图：![P14 发起页世界选区](assets/2026-07-06-P14-prototypes/compose-world-selector-desktop-mobile.png)

验收步骤：

1. 打开 `/compose`。
2. 输入正文。
3. 在平台区选择微博或微博 + Reddit。
4. 在世界区选择“新建世界”或“继续世界”。
5. 选择继续世界时，确认列表可搜索、有限展示，不随历史数量无限铺满页面。
...
参考图：![P14 世界总览](assets/2026-07-06-P14-prototypes/world-overview-desktop-mobile.png)
...
参考图：![P13 统一历史](assets/2026-07-06-P13-prototypes/unified-history-desktop-mobile.png)
...
参考图：![单平台评论区当前截图](assets/2026-07-04-current-state/single-live.png)
...
参考图：![P13 多平台现场](assets/2026-07-06-P13-prototypes/multiplatform-live-desktop-mobile.png)
...
参考图：![P14 身份页到达态](assets/2026-07-06-P14-prototypes/identity-arrival-desktop-mobile.png)
...
- 单 run 当前截图：![单 run 复盘当前截图](assets/2026-07-04-current-state/retro.png)
- Launch 原型：![P13 Launch 复盘](assets/2026-07-06-P13-prototypes/launch-retro-desktop-mobile.png)
...
- 背景、文字、强调色来自 `frontend/src/design/tokens.ts`。
- P13-T8 后的语义色不回退成大面积 slate/blue/purple。
- 白卡、深色顶栏、brand 强调、8px 圆角保持同一产品语言。
...
| 世界总览 | 身份页 | 点击主身份名 | 断言进入 `/identity/{person_id}?world_id={world_id}` |
| 历史记录 | 身份页 | 点击历史作者名 | 断言进入身份页 |
...
- 多平台现场不带 `run_id` 时是世界全景，不应用来判断一次发起是否串数据。

## 16. 设计者审核清单

- [ ] P14 原型图与当前实现的信息架构一致。
- [ ] 发起页“新建/继续世界”不会增加用户理解成本。
- [ ] 世界总览能解释长期世界和本次发起的区别。
...
- 现有 current-state 截图来自 2026-07-04，P14 以原型图和自动化验收覆盖接线变化；后续如需视觉二审，应补一组 P14 当前截图。
